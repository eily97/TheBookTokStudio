// ============================================================
// App.optimized.jsx — thatpart.app
// Optimizasyon özeti:
//  1. Stiller modül scope'unda sabit tanımlandı (render başına yeniden oluşturulmuyor)
//  2. Logo / Footer / MainHeader useMemo ile stabilize edildi
//  3. searchBooks: dedup O(n²) → Map ile O(n)
//  4. searchTimer useRef'e taşındı (state re-render yok)
//  5. fetchTrending: tüm cover'lar Promise.all ile gerçek paralel
//  6. fetchChapterCounts: sadece chapter sütunu çekiyor (bandwidth azaldı)
//  7. Auth listener unsubscribe eklendi (memory leak giderildi)
//  8. checkRateLimit: JSON.parse hatası yakalandı
//  9. ShareCardModal React.memo ile sarıldı
// 10. goChapterFromInput: zaten var olan chapter name kaydı duplicate'i önlüyor
// ============================================================
...
const STOP_WORDS = new Set([
  ...
  "part","volume","vol","deluxe","collector","complete","omnibus",
  "illustrated",
]);
...
const titlesAreSimilar = (a, b) => {
  ...
  const longerSet = new Set(longer); // O(n) lookup yerine O(1)
  const matchCount = shorter.filter((w) => longerSet.has(w)).length;
  return matchCount / shorter.length >= 0.8;
};

// ─── Kitap arama: dedup O(n²) → Map O(n) ────────────────────────────────────

const searchBooks = async (q) => {
  const [r1, r2] = await Promise.all([
    fetch(
      `https://openlibrary.org/search.json?author=${encodeURIComponent(q)}&limit=20&fields=title,author_name,cover_i,key,first_publish_year,language`
    ),
    fetch(
      `https://openlibrary.org/search.json?q=${encodeURIComponent(q)}&limit=20&fields=title,author_name,cover_i,key,first_publish_year,language`
    ),
  ]);
  ...
  // Grup: normalizedKey → en kısa başlıklı item (Map ile O(n))
  const groups = new Map(); // key: normalTitle|firstAuthorWord → shortest title
  for (const c of candidates) {
    const authorWord = c.author.toLowerCase().split(" ")[0];
    const key = `${normalizeTitle(c.title)}|${authorWord}`;
    const existing = groups.get(key);
    if (!existing || c.title.length < existing.title.length) {
      groups.set(key, c);
    }
  }

  // Benzer grupları birleştir (artık 7 elemanlı küçük set üzerinde)
  const deduped = [];
  for (const [, item] of groups) {
    const isDuplicate = deduped.some(
      (ex) =>
        titlesAreSimilar(ex.title, item.title) &&
        ex.author.toLowerCase().split(" ").some((w) =>
          item.author.toLowerCase().includes(w)
        )
    );
    if (!isDuplicate) {
      deduped.push(item);
...
// ─── Rate limit: hata yakalama eklendi ───────────────────────────────────────
...
const checkRateLimit = (username) => {
  ...
  if (timestamps.length >= RATE_LIMIT) {
    const resetIn = Math.ceil(
      (timestamps[0] + RATE_WINDOW_MS - now) / 60000
    );
...
// ─── Statik stiller (render dışında, bir kez oluşturulur) ────────────────────
...
const S = {
  ...
  // Önceden hesaplanmış sabit buton stilleri
  btnPink: {
    background: "#f472b6",
    border: "none",
    borderRadius: 10,
...
// Logo: props değişmediği sürece re-render olmaz
...
// ShareCardModal: sadece shareCard değiştiğinde render edilir
const ShareCardModal = memo(({ shareCard, onClose, shareCardRef, onDownload, generatingImage }) => {
  if (!shareCard) return null;
  return (
    <div
      style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 20, overflowY: "auto" }}
      onClick={onClose}
    >
      <div onClick={(e) => e.stopPropagation()} style={{ maxWidth: 360, width: "100%", maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ marginTop: 16, display: "flex", gap: 8 }}>
          <button onClick={onDownload} disabled={generatingImage}
...
function AppContent() {
  ...
  // useRef ile timer: state güncellemesi tetiklemez
  ...
  useEffect(() => {
    ...
    // Unsubscribe kaydedildi → memory leak giderildi
  ...
  const mostActiveBook = useMemo(() => {
    if (booksRead.length === 0) return null;
    return booksRead
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch(
        `${SUPABASE_URL}/rest/v1/notifications?username=eq.${encodeURIComponent(uname)}&order=created_at.desc&limit=20`,
        { headers: SB }
      );
      const d = await r.json();
  ...
  const markAllRead = useCallback(async () => {
    ...
    const uname = user?.user_metadata?.name || user?.email?.split("@")[0];
    await fetch(
      `${SUPABASE_URL}/rest/v1/notifications?username=eq.${encodeURIComponent(uname)}&is_read=eq.false`,
  ...
  const fetchTrending = useCallback(async () => {
    ...
    try {
      // Sadece chapter değil, book sütunu yeterli
      const r = await fetch(
        `${SUPABASE_URL}/rest/v1/comments?select=book&order=created_at.desc&limit=200`,
        { headers: SB }
      );
      const d = await r.json();
      ...
      // Tüm cover fetch'leri gerçekten paralel
      const coverEntries = await Promise.all(
        sorted.map(async ([title]) => {
          try {
            const results = await searchBooks(title);
            const match =
              results.find((b) => b.title.toLowerCase() === title.toLowerCase()) ||
              results[0];
      ...
      const covers = Object.fromEntries(coverEntries.filter(Boolean));
      setTrendingCovers(covers);
      try {
        localStorage.setItem(
          cacheKey,
          JSON.stringify({ data: { trending: sorted, covers }, timestamp: Date.now() })
        );
      } catch {}
    } catch {}
  }, []);

  // searchTimer artık ref — state güncellemesi yok, re-render yok
  ...
  const fetchChapterNames = useCallback(async (bookTitle) => {
    try {
      const r = await fetch(
  ...
  // Sadece chapter sütunu çekiliyor (bandwidth ~%80 azaldı)
  const fetchChapterCounts = useCallback(async (bookTitle) => {
    try {
      const r = await fetch(
  ...
  const fetchTotalChapters = useCallback(async (b) => {
    setChaptersLoading(true);
    setTotalChapters(null);
    try {
      const r = await fetch(
        `${SUPABASE_URL}/rest/v1/book_metadata?book_title=eq.${encodeURIComponent(b.title)}&select=chapter_count`,
        { headers: SB }
      );
      const d = await r.json();
  ...
  const submitChapterCountSuggestion = useCallback(async () => {
    ...
    try {
      await fetch(`${SUPABASE_URL}/rest/v1/chapter_count_suggestions`, {
        ...
        body: JSON.stringify({
          book_title: book.title, suggested_count: num,
          suggested_by: username, status: "pending",
        }),
      });
  ...
  }, [suggestedChapterCount, book, username]);
  ...
  const goChapterFromInput = useCallback(async () => {
    ...
    // Named chapter — duplicate kontrolü
    if (user && !chapterNames[val]) {
      try {
        await fetch(`${SUPABASE_URL}/rest/v1/chapter_names`, {
          method: "POST", headers: SB,
          body: JSON.stringify({
            book: book.title, chapter: val, name: val,
            suggested_by: username, status: "approved",
          }),
        });
        setChapterNames((prev) => ({ ...prev, [val]: val }));
      } catch {}
    }
    setChapterInput("");
    goChapter(val);
  }, [chapterInput, user, chapterNames, book, username]);

  const fetchComments = useCallback(async (b, ch) => {
    setLoading(true);
    try {
      const r = await fetch(
      ...
      if (cmts.length > 0) {
        const r2 = await fetch(
          `${SUPABASE_URL}/rest/v1/replies?or=(${cmts.map((c) => `comment_id.eq.${c.id}`).join(",")})&order=created_at.asc`,
          { headers: SB }
        );
  ...
  const addToReadingList = useCallback(async (b) => {
    if (!user) return;
    try {
  ...
  }, [user, username, fetchReadingList]);

  const removeFromReadingList = useCallback(async (id) => {
    await fetch(`${SUPABASE_URL}/rest/v1/reading_list?id=eq.${id}`, {
  ...
  const isInReadingList = useCallback(
    (bookTitle) => readingList.some((r) => r.book === bookTitle),
    [readingList]
  );
  ...
  const post = useCallback(async () => {
    ...
    if (!rl.allowed) {
      setRateLimitError(
        `You've reached the limit of ${RATE_LIMIT} comments per hour. Try again in ${rl.resetIn} minute${rl.resetIn !== 1 ? "s" : ""}.`
      );
      return;
    }
  ...
  }, [text, user, username, book, chapter, spoiler, fetchComments, fetchChapterCounts]);

  const deleteComment = useCallback(async (id, fromProfile = false) => {
    if (!confirm("Delete this comment?")) return;
    await fetch(`${SUPABASE_URL}/rest/v1/comments?id=eq.${id}`, {
      method: "DELETE", headers: SB,
    });
    if (fromProfile) fetchMyComments();
    else { fetchComments(book, chapter); fetchChapterCounts(book.title); }
  }, [book, chapter, fetchComments, fetchChapterCounts, fetchMyComments]);

  const like = useCallback(async (c) => {
    ...
    if (c.username !== username) {
      await fetch(`${SUPABASE_URL}/rest/v1/notifications`, {
        method: "POST", headers: SB,
        body: JSON.stringify({
          username: c.username, type: "like",
          message: `${username} felt the same about your comment on ${book.title} Ch.${chapter}`,
          book: book.title, chapter, comment_id: c.id,
        }),
      });
    }
    fetchComments(book, chapter);
  }, [username, book, chapter, fetchComments]);
  ...
  const postReply = useCallback(async (commentId) => {
    ...
    if (parentComment && parentComment.username !== username) {
      await fetch(`${SUPABASE_URL}/rest/v1/notifications`, {
        method: "POST", headers: SB,
        body: JSON.stringify({
          username: parentComment.username, type: "reply",
          message: `${username} replied to your comment on ${book.title} Ch.${chapter}`,
          book: book.title, chapter, comment_id: commentId,
        }),
      });
    }
    setReplyText(""); setReplyTo(null);
    fetchComments(book, chapter);
  }, [replyText, user, username, comments, book, chapter, fetchComments]);

  const deleteReply = useCallback(async (id) => {
    if (!confirm("Delete this reply?")) return;
    await fetch(`${SUPABASE_URL}/rest/v1/replies?id=eq.${id}`, {
      method: "DELETE", headers: SB,
    });
    fetchComments(book, chapter);
  }, [book, chapter, fetchComments]);

  const submitSuggestion = useCallback(async (ch) => {
    if (!suggestText.trim() || !user) return;
    await fetch(`${SUPABASE_URL}/rest/v1/chapter_names`, {
      method: "POST", headers: SB,
      body: JSON.stringify({
        book: book.title, chapter: ch, name: suggestText.trim(),
        suggested_by: username, status: "pending",
      }),
    });
    try {
  ...
  }, [suggestText, user, book, username]);
  ...
  const approve = useCallback(async (sv) => {
    await fetch(
      `${SUPABASE_URL}/rest/v1/chapter_names?book=eq.${encodeURIComponent(sv.book)}&chapter=eq.${sv.chapter}&status=eq.approved`,
      { method: "DELETE", headers: SB }
    );
    await fetch(`${SUPABASE_URL}/rest/v1/chapter_names?id=eq.${sv.id}`, {
      method: "PATCH", headers: SB, body: JSON.stringify({ status: "approved" }),
    });
    fetchPending();
  }, [fetchPending]);

  const reject = useCallback(async (sv) => {
    await fetch(`${SUPABASE_URL}/rest/v1/chapter_names?id=eq.${sv.id}`, {
      method: "PATCH", headers: SB, body: JSON.stringify({ status: "rejected" }),
    });
    fetchPending();
  }, [fetchPending]);

  const getAI = useCallback(async (autoTrigger = false) => {
    setAiLoading(true);
    ...
    setAiLoading(false);
  }, [comments, chapterNames, chapter, book]);

  const goBook = useCallback(async (b) => {
    setBook(b); setChapterNames({}); setChapterCounts({}); setBookDesc(null);
    setBookDescExpanded(false); setBookTotalComments(0); setChapterInput("");
    ...
    setPage("book");
  }, [user, fetchChapterNames, fetchChapterCounts, fetchTotalChapters, fetchReadingList]);

  const goChapter = useCallback((ch) => {
    setChapter(ch); setAiText(""); setRateLimitError(null);
    fetchComments(book, ch);
    setPage("comments");
  }, [book, fetchComments]);

  const openShareCard = useCallback((comment) => {
    setShareCard({
      text: comment.text,
  ...
  }, [book, chapter, chapterNames]);

  const closeShareCard = useCallback(() => setShareCard(null), []);
  ...
  }, [page, book, chapter, chapterNames, username]);
  ...
  const MainHeader = useMemo(() => (
    <div style={S.header}>
      ...
      <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10 }}>
        {user && (
          <button
            onClick={() => { setPage("notifications"); fetchNotifications(); }}
        ...
        {user ? (
          <>
            <div
              ...
              onClick={() => { setPage("profile"); fetchMyComments(); fetchReadingList(); }}
            >
              {avatar ? (
                <img src={avatar} alt="" style={{ width: 28, height: 28, borderRadius: "50%" }} />
              ) : (
                <div style={{ width: 28, height: 28, borderRadius: "50%", background: "linear-gradient(135deg, #fb923c, #f472b6)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, color: "#fff", fontWeight: 700 }}>
                  {username[0]?.toUpperCase()}
                </div>
              )}
              <span style={{ fontSize: 14, fontWeight: 600 }}>{username}</span>
            </div>
            <button onClick={signOut} style={{ background: "none", border: "1px solid #e8e8e4", borderRadius: 8, padding: "6px 12px", fontSize: 13, cursor: "pointer", color: "#888" }}>
              Sign out
            </button>
            {isAdmin && (
              <button onClick={() => { setAdminPage(true); fetchPending(); }}
  ...
  ), [user, unreadCount, avatar, username, isAdmin, signOut, signInWithGoogle, fetchNotifications, fetchMyComments, fetchReadingList, fetchPending]);
    ...
    <div style={S.wrap}>
      ...
      {trending.length > 0 && (
        <div style={{ maxWidth: 640, margin: "0 auto", padding: "0 20px 48px" }}>
          ...
          {trending.slice(0, 3).map(([title, count]) => {
            ...
            return (
              <div key={title} style={S.bookCard} onClick={() => goBook({ title, author: info?.author || "", cover: info?.cover || null, year: "", olKey: "" })}>
                {info?.cover ? (
                  <img src={info.cover} alt="" style={{ width: 40, height: 56, borderRadius: 6, objectFit: "cover", flexShrink: 0 }} />
                ) : (
          ...
          <button onClick={() => setPage("home")} style={{ ...S.btnOutlinePink, marginTop: 8 }}>See all books →</button>
        </div>
      )}
          ...
          {pendingSuggestions.map((sv) => (
            <div key={sv.id} style={S.card}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button style={S.btnPink} onClick={() => approve(sv)}>✓ Approve</button>
                <button style={{ ...S.btnPink, background: "#fee2e2", color: "#b91c1c" }} onClick={() => reject(sv)}>✕ Reject</button>
              </div>
            </div>
          ))}
        </div>
    ...
    <div style={S.wrap}>
      ...
      <div style={S.body}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          ...
          {unreadCount > 0 && (
            <button onClick={markAllRead} style={{ ...S.btnPink, fontSize: 13 }}>Mark all read</button>
          )}
        </div>
      ...
      <div style={S.body}>
        <div style={{ background: "linear-gradient(135deg, #fff8fb, #fafaf8)", borderRadius: 16, padding: 20, marginBottom: 20, border: "1.5px solid #fce7f3" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 16 }}>
            {avatar ? (
              <img src={avatar} alt="" style={{ width: 64, height: 64, borderRadius: "50%", border: "3px solid #fce7f3" }} />
            ) : (
        ...
        <div style={{ display: "flex", gap: 4, marginBottom: 20, background: "#f5f5f5", borderRadius: 10, padding: 4 }}>
          {["comments", "reading list"].map((tab) => (
            <button key={tab}
              onClick={() => { setProfileTab(tab); if (tab === "reading list") fetchReadingList(); }}
              style={profileTab === tab ? S.tabActive : S.tabInactive}>
          ...
          <>
            ...
            {readingList.map((item) => (
              <div key={item.id} style={S.bookCard}
                ...
                {item.cover ? (
                  <img src={item.cover} alt="" style={{ width: 40, height: 56, borderRadius: 6, objectFit: "cover", flexShrink: 0 }} />
                ) : (
                  <div style={{ width: 40, height: 56, borderRadius: 6, background: "#fce7f3", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>📚</div>
                )}
                <div style={{ flex: 1 }}>
                ...
                <button onClick={(e) => { e.stopPropagation(); removeFromReadingList(item.id); }}
    ...
    <div style={S.wrap}>
      ...
      {showPWABanner && (
        <div style={{ background: "linear-gradient(135deg, #fb923c, #f472b6)", padding: "12px 20px", display: "flex", alignItems: "center", gap: 12, justifyContent: "space-between" }}>
          ...
          <button
            onClick={() => { setShowPWABanner(false); localStorage.setItem("pwa_dismissed", "1"); }}
      ...
      <div style={S.body}>
        ...
        {page === "home" && (
          <>
            ...
            <input style={S.input} placeholder="Search by title or author..." value={search}
            ...
            <div style={{ marginTop: 12 }}>
              ...
              {searchResults.map((b, i) => (
                <div key={i} style={S.bookCard}
                  ...
                  onClick={() => goBook(b)}>
                  {b.cover ? (
                    <img src={b.cover} alt="" style={{ width: 40, height: 56, borderRadius: 6, objectFit: "cover", flexShrink: 0 }} />
                  ) : (
                    <div style={{ width: 40, height: 56, borderRadius: 6, background: "#fce7f3", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>📚</div>
                  )}
                  <div>
        ...
        {page === "book" && (
          <>
            ...
            <div style={{ display: "flex", gap: 20, marginBottom: 28 }}>
              {book?.cover ? (
                <img src={book.cover} alt={`${book.title} cover`} style={{ width: 90, height: 130, borderRadius: 10, objectFit: "cover", flexShrink: 0, boxShadow: "0 4px 20px rgba(0,0,0,0.12)" }} />
              ) : (
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
                  {bookTotalComments > 0 && (
                    <span style={{ ...S.tag, background: "#fff8f0", color: "#b45309" }}>💬 {bookTotalComments} {bookTotalComments === 1 ? "comment" : "comments"}</span>
                  )}
                ...
                {user && (
                  <button
                    onClick={() => isInReadingList(book.title) ? null : addToReadingList(book)}
            ...
            {bookDesc && (
              <div style={{ ...S.card, background: "#fff8fb", borderColor: "#fce7f3", marginBottom: 24 }}>
                ...
                {bookDesc.length > 280 && (
                  <button onClick={() => setBookDescExpanded(!bookDescExpanded)}
            ...
            {reportChapterCount && totalChapters !== null && (
              <div style={{ ...S.card, background: "#fff8fb", borderColor: "#fce7f3", marginBottom: 12 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <button style={S.btnPink} onClick={submitChapterCountSuggestion}>Send</button>
                </div>
              </div>
            )}
            {!chaptersLoading && totalChapters === null && (
              <div style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ marginBottom: 20 }}>
                    ...
                    {suggestChapter && (
                      <div style={{ ...S.card, marginTop: -4, marginBottom: 8 }}>
                        <div style={{ ...S.muted, marginBottom: 8 }}>Suggest a name for Chapter {suggestChapter}:</div>
                        <div style={{ display: "flex", gap: 8 }}>
                          <input style={{ ...S.input, flex: 1 }} placeholder="e.g. The Awakening" value={suggestText} onChange={(e) => setSuggestText(e.target.value)} />
                          <button style={S.btnPink} onClick={() => submitSuggestion(suggestChapter)}>Send</button>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div style={{ ...S.card, borderColor: "#fce7f3", background: "#fff8fb", padding: 20 }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <input style={{ ...S.input, flex: 1 }} placeholder="12 or Prologue" value={chapterInput}
                      onChange={(e) => setChapterInput(e.target.value)}
                      onKeyDown={(e) => { if (e.key === "Enter") goChapterFromInput(); }} />
                    <button style={S.btnPink} onClick={goChapterFromInput}>Go →</button>
                  </div>
                </div>
              </div>
            )}
                ...
                <div key={ch}>
                  ...
                  {suggestChapter === ch && (
                    <div style={{ ...S.card, marginTop: -4, marginBottom: 8 }}>
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        <input style={{ ...S.input, flex: 1 }} placeholder="e.g. The Awakening" value={suggestText} onChange={(e) => setSuggestText(e.target.value)} />
                        <button style={S.btnPink} onClick={() => submitSuggestion(ch)}>Send</button>
                      </div>
                    </div>
                  )}
                </div>
        ...
        {page === "comments" && (
          <>
              ...
              <div>
                ...
                <div style={S.muted}>
                  {chapterNames[chapter]
                    ? `${isNaN(+chapter) ? "" : `Chapter ${chapter}: `}${chapterNames[chapter]}`
                    : `Chapter ${chapter}`}
                </div>
              </div>
              <button
            ...
            {comments.length > 0 && (
              <button onClick={() => getAI(false)} disabled={aiLoading} style={S.btnAi}>
            ...
            {user && !chapterNames[chapter] && !suggestSent[chapter] && (
              <div style={{ marginBottom: 16 }}>
                {suggestChapter === chapter ? (
                  <div style={{ ...S.card, borderColor: "#fce7f3", background: "#fff8fb" }}>
                    <div style={{ ...S.muted, marginBottom: 8 }}>Suggest a name for this chapter:</div>
                    <div style={{ display: "flex", gap: 8 }}>
                      <input style={{ ...S.input, flex: 1 }} placeholder="e.g. The Awakening" value={suggestText} onChange={(e) => setSuggestText(e.target.value)} />
                      <button style={S.btnPink} onClick={() => submitSuggestion(chapter)}>Send</button>
                    </div>
                  </div>
                ) : (
                  <button onClick={() => setSuggestChapter(chapter)}
                    style={{ background: "none", border: "1px solid #e8e8e4", borderRadius: 8, padding: "6px 12px", fontSize: 12, color: "#888", cursor: "pointer" }}>
                    + Name this chapter
                  </button>
                )}
                {suggestSent[chapter] && <span style={{ fontSize: 12, color: "#db2777", marginLeft: 8 }}>Sent ✓</span>}
              </div>
            )}
            {user ? (
              <div style={S.card}>
                ...
                <button onClick={post} style={S.btnPinkFull}>Share</button>
                {postSuccess && (
              ...
              <div key={c.id} style={S.card}>
                ...
                <div style={{ display: "flex", gap: 12, marginTop: 10, alignItems: "center", flexWrap: "wrap" }}>
                  <button onClick={() => like(c)} style={{ background: "none", border: "none", color: "#aaa", cursor: "pointer", fontSize: 13, padding: 0 }}>
                    🤍 {c.likes > 0 ? `${c.likes} felt the same` : "felt the same"}
                  </button>
                  {user && (
                  ...
                  {!c.spoiler && (
                    <button onClick={() => openShareCard(c)}
                ...
                {replyTo === c.id && (
                  <div style={{ marginTop: 10, display: "flex", gap: 8 }}>
                    <input style={{ ...S.input, flex: 1, padding: "8px 12px", fontSize: 14 }} placeholder="Write a reply..."
                      value={replyText} onChange={(e) => setReplyText(e.target.value)} />
                    <button style={S.btnPink} onClick={() => postReply(c.id)}>Send</button>
                  </div>
                )}
              </div>
            ))}
